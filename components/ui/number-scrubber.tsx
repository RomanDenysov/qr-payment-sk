...
>(
  (
    ...
    function clampAndQuantize(n: number) {
      ...
      return Number.parseFloat(clamped.toFixed(decimals));
    }
      ...
      <div className={cn('group relative w-fit')}>
        <Input
          ...
          className={cn(
            ...
            `pr-0 [appearance:textfield] hover:cursor-ew-resize active:cursor-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none `,
            className
          )}
          step={step}
          value={internalValue}
        ...
        <div
          className="absolute inset-y-0 right-0 flex cursor-ew-resize select-none items-center px-2 text-gray-400"
          onPointerDown={handlePointerDown}
        >
          ↔
        </div>
